...
app.post('/auth/register', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO users (email, username, password, balance) VALUES ($1, $2, $3, $4) RETURNING id, email, username, balance',
      [email, email.split('@')[0], hashedPassword, 1000.00]
    );
...
app.get('/auth/profile', verifyToken, async (req, res) => {
  try {
    const result = await pool.query('SELECT id, email, username, balance FROM users WHERE id = $1', [req.user.id]);
